import { CircleUserRound, Copy, DoorClosed, KeyRound, ListMusic, LoaderCircle, LogIn, LogOut, MonitorCog, Moon, Music, Network, RefreshCw, Save, Settings2, ShieldCheck, Sun, SunMoon, Trash2, Upload, Users, X } from 'lucide-react'
import { useEffect, useRef, useState } from 'react'
import { canDirectly } from '../domain/permissions'
import type { AdminRoom, AdminUser, AudioQuality, MusicSource, Playlist, Track } from '../domain/types'
import { fetchAdminRooms, fetchAdminUsers, fetchAudioProxyPolicy, fetchPlaylistTracks, requestJson } from '../services/api'
import { addBatchToQueue, checkPlatformQr, claimKugouConceptVip, loginAccount, logoutAccount, logoutPlatform, protectAccount, refreshProfile, renameAccountId, requestMyPlaylists, requestPlatformQr, saveAvatar, saveNickname, setPlatformCookie, setRoomUserRole, updateLyricSettings, updateRoomSettings, updateSyncInterval } from '../services/runtime'
...
const QUALITY_OPTIONS: Array<{ value: AudioQuality; label: string }> = [
  { value: 128, label: '标准 128 kbps' }, { value: 320, label: '高品质 320 kbps' }, { value: 999, label: '无损优先' },
...
function RoomSettings() {
  ...
  return <div className="settings-section"><SectionHeader title="房间与成员" description="房间状态由服务器保存；临时管理员可控制播放和队列，成员使用投票。" />
    <div className="settings-group">
      ...
      <SettingRow label="房间号" description="可直接发送给其他客户端"><div className="copy-field"><code>{room.id}</code><button className="icon-button" title="复制房间号" onClick={() => { void navigator.clipboard.writeText(room.id); notify('房间号已复制') }}><Copy size={14} /></button></div></SettingRow>
      ...
      <SettingRow label="密码保护" description={room.hasPassword ? '当前房间已设置密码' : '当前房间公开加入'}><div className="inline-editor"><input type="password" disabled={!owner} value={password} onChange={(event) => setPassword(event.target.value)} placeholder="输入新密码" /><button className="button" disabled={!owner || !password.trim()} onClick={() => { updateRoomSettings({ password: password.trim() }); setPassword('') }}>设置</button>{room.hasPassword && <button className="button" disabled={!owner} onClick={() => updateRoomSettings({ password: null })}>移除</button>}</div></SettingRow>
    </div>
    {!owner && <div className="permission-note">当前身份：{user?.role === 'admin' ? '临时管理员，可控制播放、队列和音质' : '成员，可点歌并对受限操作发起投票'}</div>}
    <div className="settings-group members-settings"><h3>在线成员 <span>{room.users.length}</span></h3>{room.users.map((member) => <div className="settings-member" key={member.id}>{member.avatarUrl ? <img src={resolveAsset(member.avatarUrl)} alt="" /> : <span>{member.nickname.slice(0, 1).toUpperCase()}</span>}<div><strong>{member.nickname}{member.id === userId ? '（你）' : ''}</strong><code>{member.id}</code></div><em>{member.isServerAdmin ? '服务器管理员' : member.role === 'owner' ? '房主' : member.role === 'admin' ? '管理员' : '成员'}</em>{owner && member.role !== 'owner' && !member.isServerAdmin && member.id !== userId && <select value={member.role} onChange={(event) => setRoomUserRole(member.id, event.target.value as 'admin' | 'member')}><option value="admin">管理员</option><option value="member">成员</option></select>}</div>)}</div>
  </div>
}
...
function PlaybackSettings() {
  ...
  return <div className="settings-section"><SectionHeader title="播放与同步" description="客户端定期向服务器请求权威播放时间；大偏差会自动校正。" /><div className="settings-group"><SettingRow label="连接状态"><span className={`connection-pill connection-pill--${status}`}>{status === 'connected' ? '已连接' : status === 'reconnecting' ? '正在重连' : status === 'connecting' ? '正在连接' : '已断开'}</span></SettingRow><SettingRow label="当前同步偏差"><code className={Math.abs(drift) > 500 ? 'drift-high' : ''}>{drift > 0 ? '+' : ''}{drift} ms</code></SettingRow><SettingRow label="同步请求间隔" description="3-60 秒；网络不稳定时可缩短"><div className="range-setting"><input type="range" min={3} max={60} value={interval} onChange={(event) => updateSyncInterval(Number(event.target.value))} /><strong>{interval} 秒</strong></div></SettingRow></div></div>
}
