...
新曲在元数据就绪后等待服务端计划时间，并在实际开始时重新估算进度。切歌、离开、断线及身份连接重建取消旧播放调度；可选 revision 用于忽略较旧播放动作。校时每 30 秒补充样本并优先使用低 RTT 样本的偏移中位数；硬 Seek 独立于变速开关，连续三次超限才执行。

连接初始化、搜索与推荐请求具有过期结果保护。services/lyrics.ts 独立负责 TTML/平台歌词并行加载、超时、取消、逐字优先展示及按服务器和歌词设置隔离的 30 项/30 分钟缓存；顺序和列表循环预取下一曲，随机及单曲循环不预测下一曲。
