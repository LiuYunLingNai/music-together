...
解析顺序与 Android 客户端一致：TTML、服务端逐词、YRC、LRC。TTML 与平台歌词先按逐字动画、有效时间、正文完整度、重复副歌覆盖和背景人声/对唱结构综合选择；高质量逐字歌词缺少外语译文或正文、副歌不完整时，再调用服务端多歌词源补全。辅助译文和音译按全局顺序、正文相似度及稳定时间偏移对齐，只填空缺字段，不改写 Windows AMLL 使用的行时间、逐字时间、Ruby 和已有背景/对唱标记；结构迁移仅在 TTML 与选中时间轴达到足够覆盖时启用。
...
连接初始化、搜索与推荐请求具有过期结果保护。services/lyrics.ts 独立负责 TTML/平台歌词并行加载、超时、取消、质量选择、多源补全及按服务器和歌词设置隔离的 30 项/30 分钟缓存；顺序和列表循环通过同一加载链路预取下一曲，随机及单曲循环不预测下一曲。
