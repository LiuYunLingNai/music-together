...
服务器管理员通过受保护的 HTTP API 分别控制 B 站与酷狗是否强制代理，策略通过 `server:audio_proxy_policy` 实时下发。默认及旧服务端兼容行为均为强制代理。

关闭强制代理后，Android 对相应来源优先使用服务端解析出的 CDN 地址，并保留服务器代理地址作为一次性回退。直连加载失败时 `NativePlayer` 保持当前进度切换到代理；重新开启强制代理时，正在直连的对应曲目也会切回代理。酷狗策略同时覆盖标准版和概念版，概念版加密资源由回退代理完成流式解密。其他来源继续使用服务端下发的直连地址。
