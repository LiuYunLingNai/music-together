...
| 视觉舞台   | 经典播放器之外可选 Mineradio 沉浸式舞台（Emily/滚筒/星球/唱片/星河/声波地形）；全屏 three.js 场景（封面采样粒子 + 世界空间歌词网格）+ 底部玻璃控制台。歌词在 WebGL 中实现逐字/对唱/背景行/译词音译，并保留 AMLL 作为可切换回退。切舞台不触碰播放与同步逻辑，经典播放器始终可无损回退 |
