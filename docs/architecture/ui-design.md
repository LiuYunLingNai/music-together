...
- **AMLL** (`@applemusic-like-lyrics/react`)：Apple Music 风格逐行/逐词歌词动画；歌词设置可控制渐变宽度、已播放行隐藏、敏感词遮罩和末尾歌曲信息，同时保留逐曲时间偏移校准。末尾歌曲信息使用 AMLL 官方 `bottomLine` 插槽，不进入可播放歌词时间轴。播放器准备音频时即预取歌词，当前平台歌词与 TTML 并行加载，并使用短暂质量等待窗口避免为较慢的优质候选长期阻塞首屏；最终结果存入客户端有界缓存
- **多源逐词择优**：当前平台没有通过质量门槛的逐词歌词，或 TTML 仍有无效时间行时，服务端才并行搜索其他平台。候选必须通过标准化歌名、全部歌手和 ±3 秒时长校验，避免误用翻唱、Live 或同名歌曲。客户端以忽略制作信息和平台断句差异的整首有序文本为共同基准，统一评估实际逐词动画覆盖率、歌词完整度、有效时间轴覆盖率和独立计时行数量；带有 AMLL 对唱或背景人声结构的完整候选优先保留，无结构的 YRC/KRC 只能用于补全，不能整份覆盖。普通 LRC 仅用于时间轴修复或最终回退，不会伪装成逐词动画
- **逐词来源**：TTML 在线库（网易云/QQ）、平台原生 YRC（网易云）、平台原生 KRC（酷狗，服务端解析后以 `wordByWord` 返回）。TTML 只是候选之一，不是跨平台补充的前提
- **时间轴修复**：无有效时长的 TTML 行可使用同源或严格匹配的跨平台逐词/LRC 候选补全；仅替换异常行并估算来源整体偏移，随后以“主歌词＋附属背景人声”为组稳定排序，避免破坏既有逐字、对唱、短语气词和背景人声结构
