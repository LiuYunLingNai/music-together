...
- 新增或修改导致单个文件持续膨胀时，先识别可独立维护的职责；边界明确的代码优先拆分为内聚模块，并保留清晰的入口和依赖方向，避免继续向大型文件堆叠实现
- `AppState` 及其子状态使用不可变数据类和 `copy()` 更新
- 异步请求使用可取消的 `Job`，切换服务器、房间、曲目或查询时清理旧任务
- 新增服务端字段时同步更新模型、JSON 读写和往返测试

调整目录结构、模块职责、核心数据流或跨模块边界时，Agent 必须在同一任务中更新 `docs/PROJECT_ARCHITECTURE.md`。修改 Agent 工作约定、验证方式或提交发布规则时，必须同步更新 `AGENTS.md`。局部实现变化未改变文档描述时，无需改动文档。
...
标题下空一行。正文使用项目符号简要列出本次提交的主要更新点，条目数量按实际内容确定：
...
正文只记录本次提交已经完成的主要改动。兼容处理、测试补充和版本号仅在实际发生且有助于理解提交时列出，不设置固定字段，也不要求写入验证结果。验证结果保留在任务交付说明中。独立领域优先拆分提交。版本号属于同一批功能或修复时，在该提交中一并更新，并在正文列出目标版本。只有版本号或发布元数据变化时，标题按实际内容概括即可；不为发布流程额外创建空提交。
