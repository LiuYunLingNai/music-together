 ...
 *     off       51.4%     57.7%     64.3%
 *
 *   ⚠️ 下面这张表是**间距均分之前**的旧标定数据，仅作历史参照。
 *      均分后 `current` 在 10 行实测为 0.715 —— 以此为准，故阈值取 **0.72**
 *      （**不是**表中所暗示的 0.70；0.70 会把 10 行的 `current` 误降到 `off`，
 *      把用户明确要的"仅当前行译词"整个藏掉）。详见下方 `RETRACT_STACK_LIMIT`。
 *
