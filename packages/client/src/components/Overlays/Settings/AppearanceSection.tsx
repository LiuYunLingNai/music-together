...
import { VISUAL_QUALITIES, VISUAL_QUALITY_LABELS } from '@/components/Player/mineradio/shared/RenderPolicy'
import {
...
import { useVisualStageDisabled } from '@/hooks/useVisualStageSupport'
import { SettingRow } from './SettingRow'

export function AppearanceSection() {
  const s = useSettingsStore()
  /**
   * 移动端（触摸设备）不提供视觉舞台 —— 设置面板是**第二个**入口，
   * 只藏播放器里的菜单不够。见 `lib/visualStageSupport.ts`。
   */
  const visualStageDisabled = useVisualStageDisabled()
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ---- 视觉舞台 ----
          ★ 整块在触摸设备上不渲染：这里全部设置项都只作用于 Mineradio
            视觉舞台（`MineradioPlayerStage` 消费），而该舞台在移动端没有
            入口。留着这些控件会给出"改了能生效"的错误预期 —— 而它们唯一
            的作用是把存储值改成移动端用不到的舞台。
            注意「背景渲染」那一块**不属于**此列 —— 经典播放器的
            `ClassicPlayerStage` 也读 `bgFps`/`bgFlowSpeed`/`bgRenderScale`
            （`classic/ClassicPlayerStage.tsx:78-80`），必须保留。 */}
      {!visualStageDisabled && (
        <div>
          ...
          <SettingRow
            label="歌词显示行数"
            description={s.lyricRenderer === 'webgl' ? '影院为 5 行，自定义可选 1-10 行' : '仅 3D 歌词渲染器生效'}
            onReset={s.lyricDisplayMode3d !== s.lyricDisplayMode3dDefault ? s.resetLyricDisplayMode3d : undefined}
          >
            <Select
          ...
          {s.lyricDisplayMode3d === 'custom' && (
            <SettingRow
              ...
              onReset={
                s.lyricCustomLineCount !== s.lyricCustomLineCountDefault ? s.resetLyricCustomLineCount : undefined
              }
            >
              <Slider
          ...
          <SettingRow
            ...
            onReset={
              s.lyricTranslationMode3d !== s.lyricTranslationMode3dDefault ? s.resetLyricTranslationMode3d : undefined
            }
          >
            <Select
