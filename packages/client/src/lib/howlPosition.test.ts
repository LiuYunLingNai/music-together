...
describe('setHowlPosition', () => {
  it('updates a playing HTML5 media element without invoking Howler seek', () => {
    ...
    const howl = { _sounds: [sound], playing: vi.fn(() => true), seek }
  ...
  it('falls back to the public API while paused and clamps negative positions', () => {
    const seek = vi.fn()
    const howl = { _sounds: [], playing: vi.fn(() => false), seek }

    setHowlPosition(howl as never, -8, 3)

    expect(seek).toHaveBeenCalledWith(0, 3)
  })
})
